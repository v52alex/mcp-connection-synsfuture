...
class ComposeReadService:
    ...
    async def inspect(
        ...
        result = await self._run(
            profile_id, "inspect_compose_project", path, ["config", "--format", "json"]
        )
    ...
    async def ps(
        ...
        path = self._compose_path(project_path, compose_file)
        return await self._run(profile_id, "compose_ps", path, ["ps", "--format", "json"])

    async def logs(
        self, profile_id: str, project_path: str, compose_file: str | None, tail: int
    ) -> ComposeReadResult:
        if not 1 <= tail <= 500:
            raise ValueError("tail must be between 1 and 500")
        path = self._compose_path(project_path, compose_file)
        result = await self._run(profile_id, "compose_logs", path, ["logs", "--tail", str(tail)])
        return result.model_copy(
            update={"lines": [self._redact(line)[:2000] for line in result.lines[-tail:]]}
        )

    async def _run(
        self, profile_id: str, operation: str, path: Path, command: list[str]
    ) -> ComposeReadResult:
